...
- Phenotypes were simulated using the [S-LDSC baseline model enrichment estimates for human traits](https://www.nature.com/articles/ng.3404/figures/4).
    - The enrichment estimates were used to calculate variance component (VC) estimates for intercept and 24 main functional annotations.
...
3. Create a [GRM list](options.md#grm-input): [**chr.grms.txt**](https://github.com/jiang18/mph/blob/main/examples/QTL-MAS-2012/chr.grms.txt).
...
result = recalculate_enrichments(vcfile, cp, nsnps=nsnps, annot.size=annot_size) 
result
...
4. Create a [GRM list](options.md#grm-input) for `--reml`, listing A, D, AxA, AxD, and DxD: [**ADE.grms.txt**](https://github.com/jiang18/mph/blob/main/examples/QTL-MAS-2012/ADE.grms.txt).
