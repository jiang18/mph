...
If functional categories actually overlap, one more quick step is needed to recalculate PVEs and enrichments from **.mq.vc.csv** and [the SNP info file](options.md#snp-info-file). This can be quickly done by `recalculate_enrichments()` in [**mph_functs.R**](https://github.com/jiang18/mph/tree/main/scripts/mph_functs.R).
