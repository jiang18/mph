...
- `make_grms_for_pair.R` generates three GRMs for a trait pair: one for the variance of trait 1 (**.G11**), one for the variance of trait 2 (**.G22**), and one for the covariance between them (**.G12**).
- `make_rescov_for_pair.R` generates two GRMs for a trait pair: one for the difference between the environmental variances of traits 1 and 2 (**.E1d**), and the other for the environmental covariance between them (**.E12**).
